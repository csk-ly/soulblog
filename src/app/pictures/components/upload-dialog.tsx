			...
			<div className='space-y-4'>
				...
				<div>
					...
					{images.length === 0 ? (
						<div
							...
							className='flex h-32 cursor-pointer items-center justify-center rounded-xl border border-dashed border-white/20 bg-white/6 transition-colors hover:bg-secondary/10'>
						...
						<>
							<div className='relative flex h-40 items-center justify-center overflow-visible rounded-xl bg-linear-to-br from-white/6 to-white/4'>
								{images.slice(0, 3).map((image, index) =>
									image.type === 'file' ? (
										<div
											key={index}
											className={`absolute h-32 w-44 overflow-hidden rounded-xl border-4 border-white/30 bg-white/8 shadow-xl backdrop-blur-sm transition-transform ${
							...
							<div className='mt-3 flex items-center justify-between'>
								...
								<button
									type='button'
									onClick={() => fileInputRef.current?.click()}
									className='rounded-lg border border-white/20 bg-white/4 px-3 py-1.5 text-xs text-gray-700 transition-colors hover:bg-white/6'>
									继续添加
								</button>
							</div>
						</>
				...
				<div>
					...
					<textarea
						...
						placeholder='这组图片的说明...'
						className='w-full rounded-md border border-white/20 bg-white/6 px-3 py-2 text-sm focus:outline-none'
						rows={3}
					/>
				</div>
				...
				<div className='mt-4 flex gap-3'>
					<button
						type='button'
						onClick={handleClose}
						className='flex-1 rounded-lg border border-white/20 bg-white/4 px-4 py-2 text-sm transition-colors hover:bg-white/6'>
						取消
					</button>
